...
def get_outputs_data(o_data):
    ...
    data = {}
    # fake statistic data
    data['statistics'] = {"value": {"iter": ["loss", "accuracy", "precision", "recall"], "time": ["memory"]}}
    data['statistics'][[j for j in data['statistics']["value"].keys()][0]] = [i for i in range(10)]
    for i in data['statistics']["value"][[j for j in data['statistics']["value"].keys()][0]]:
        data['statistics'][i] = [random.random() for k in range(10)]
    ...
    for node_index in nodes.keys():
        ...
        if node.is_outputed:
            node.attr["shape"] = "["
            if len(node.outputs_size) != 0:
                for num in node.outputs_size[0]:
                    node.attr["shape"] += str(num) + ","
            node.attr["shape"] += "]"
            if node.parent != -1:
    ...
    count_edge = 0
    for edge in edges:
        edge.start_stack.append(edge.start)
        edge.end_stack.append(edge.end)
        nodes[edge.start].next.append(edge.end)
        nodes[edge.end].pre.append(edge.start)
        ...
        edges_outputs.append({
            "start": edge.start,
            "end": edge.end,
            # "start_stack": edge.start_stack,
            # "end_stack": edge.end_stack,
            "start_stack": [edge.start],
            "end_stack": [edge.end],
            "index": count_edge
        })
        count_edge += 1
    data['network']['edges'] = edges_outputs
    for node_index in nodes.keys():
        node = nodes[node_index]
    ...
    data['network']['nodes'] = nodes_outputs
...
def create_activation_matrix_image(rawdata, data_id, shape, var_node_id):
